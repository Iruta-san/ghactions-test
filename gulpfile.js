...
gulp.task('watch', () => {
	...
	gulp.watch([
		...
		// Если оставить как тут, то перестаёт собирать с включенным флагом cache
		// .on(['change', 'add'], (event, file) => {
		.on('all', (event, file) => {
...
// !! Задача для разработки, без билда
gulp.task('develop-watch', gulp.series(
	gulp.parallel(
		'watch',
		'serve',
	)
